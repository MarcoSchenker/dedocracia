...
VotoRegistrado votosRegistrados[100]; // Array para almacenar votos
int totalVotosRegistrados = 0;
const unsigned long TIMEOUT_VOTO = 300000; // 5 minutos en milisegundos
...
bool enrollFingerAuto(int id) {
  ...
  // CAMBIADO: Segunda imagen con 20 segundos de timeout
...
// Función para verificar si una huella ya votó recientemente
bool yaVoto(int id_huella) {
  unsigned long ahora = millis();
  
  for (int i = 0; i < totalVotosRegistrados; i++) {
    if (votosRegistrados[i].id_huella == id_huella) {
      // Si han pasado más de 5 minutos, permitir votar de nuevo
      if (ahora - votosRegistrados[i].timestamp > TIMEOUT_VOTO) {
        // Actualizar timestamp para este voto
        votosRegistrados[i].timestamp = ahora;
        return false; // Puede votar
      }
      return true; // No puede votar, muy reciente
    }
  }
  return false; // No ha votado antes
}

// Función para registrar un voto en memoria local
void registrarVotoLocal(int id_huella) {
  // Si hay espacio, agregar nuevo voto
  if (totalVotosRegistrados < 100) {
    votosRegistrados[totalVotosRegistrados].id_huella = id_huella;
    votosRegistrados[totalVotosRegistrados].timestamp = millis();
    totalVotosRegistrados++;
  } else {
    // Si no hay espacio, reemplazar el más antiguo
    int indiceAntiguo = 0;
    unsigned long tiempoAntiguo = votosRegistrados[0].timestamp;
    
    for (int i = 1; i < 100; i++) {
      if (votosRegistrados[i].timestamp < tiempoAntiguo) {
        tiempoAntiguo = votosRegistrados[i].timestamp;
        indiceAntiguo = i;
      }
    }
    
    votosRegistrados[indiceAntiguo].id_huella = id_huella;
    votosRegistrados[indiceAntiguo].timestamp = millis();
  }
}
...
void pedirVoto() {
  ...
  // CAMBIADO: Simplificado a colores de botones
  ...
  // CAMBIADO: Aumentado de 10 segundos a 60 segundos (1 minuto)
